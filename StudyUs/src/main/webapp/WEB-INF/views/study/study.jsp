...
<body>
								...
								<div class="">
									<c:forEach items="${recentNotice }" var="notice">
										<c:if test="${notice.noContents ne null }">
											<div class="card-body">${notice.noContents }</div>
										</c:if>
						...
						<div class="card">
							<div class="card-body" style="padding: 0;">
								<div class="text-center"
									style="margin-top: 15%; margin-bottom: 10%; color: #683FF7;">
									<h1>
										<c:forEach items="${attMember}" var="meeting">
										${meeting.requiredAttendance }
									</c:forEach>
										/ ${study.maxPersonnel }
									</h1>
								</div>
								...
								<div class="row">
									...
									<div class="col-md-3 d-flex flex-row-reverse">
										<c:if test="${attendanceStatus eq 0  }">
											<button id="attendance-check-button"
												class="btn btn-light btn-rounded btn-sm" disabled>출석확인</button>
											<span id="attendance-help" class="pr-4">출석시간이 아닙니다.</span>
										</c:if>
										<c:if test="${attendanceStatus eq 1 }">
											<button id="attendance-check-button"
												class="btn btn-primary btn-rounded btn-sm"
												onclick="insertAttendance(${loginUser.mbNo}, ${sessionScope.study.studyNo });">출석확인</button>
										</c:if>
										<c:if test="${attendanceStatus eq 2  }">
											<button id="attendance-check-button"
												class="btn btn-primary btn-rounded btn-sm" disabled>출석확인</button>
	...
	<script
		src="https://cdn.jsdelivr.net/npm/chart.js@3.3.2/dist/chart.min.js"></script>
	<script
		src="/resources/css/study/dist/js/pages/dashboards/dashboard1.min.js"></script>
